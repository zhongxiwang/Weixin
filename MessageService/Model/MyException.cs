...
{"2000002","	CorpId参数无效 指定的CorpId不存在"}
        ...
        public MyException(string json,bool b)
        {
            if (!b) { _msg = json; return; }
        ...
        private void SetMsg(string errorcode)
        {
            try
            {
                if (list.ContainsKey(errorcode))
                    _msg ="errorcode:"+errorcode+" "+ list[errorcode];
                if (!errorcode.Equals("0")) throw new Exception(this._msg);
            }
            catch
            {
                throw;
